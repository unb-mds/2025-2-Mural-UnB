import TB from "assets/images/fotos/devs/TB.jpeg"
import LF from "assets/images/fotos/devs/LF.jpeg"
import JG from "assets/images/fotos/devs/JG.jpeg"
import LL from "assets/images/fotos/devs/LL.jpeg"
import MS from "assets/images/fotos/devs/MS.jpeg"
import MG from "assets/images/fotos/devs/MG.jpeg"

export default function Sobre() {
    return (
        <div className="flex flex-col items-center">
            <h1 className="text-2xl font-gowunBold mt-10">Desenvolvedores</h1>

            <div className="flex flex-row -m-5">
                <div className="card bg-base-100 w-65 shadow-sm max-h-100 scale-75">
                    <figure>
                        <img
                        src={TB}
                        alt="TiagoB" />
                    </figure>
                    <div className="card-body items-center">
                        <h1 className="card-title pb-2 font-gowunBold">Tiago Bittencourt</h1>
                        <div className="card-actions items-center">
                        <button className="btn btn-circle bg-black text-white border-black">
                            <a href="https://github.com/TiagoSBittencourt">
                                <svg aria-label="GitHub logo" width="32" height="32" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="white" d="M12,2A10,10 0 0,0 2,12C2,16.42 4.87,20.17 8.84,21.5C9.34,21.58 9.5,21.27 9.5,21C9.5,20.77 9.5,20.14 9.5,19.31C6.73,19.91 6.14,17.97 6.14,17.97C5.68,16.81 5.03,16.5 5.03,16.5C4.12,15.88 5.1,15.9 5.1,15.9C6.1,15.97 6.63,16.93 6.63,16.93C7.5,18.45 8.97,18 9.54,17.76C9.63,17.11 9.89,16.67 10.17,16.42C7.95,16.17 5.62,15.31 5.62,11.5C5.62,10.39 6,9.5 6.65,8.79C6.55,8.54 6.2,7.5 6.75,6.15C6.75,6.15 7.59,5.88 9.5,7.17C10.29,6.95 11.15,6.84 12,6.84C12.85,6.84 13.71,6.95 14.5,7.17C16.41,5.88 17.25,6.15 17.25,6.15C17.8,7.5 17.45,8.54 17.35,8.79C18,9.5 18.38,10.39 18.38,11.5C18.38,15.32 16.04,16.16 13.81,16.41C14.17,16.72 14.5,17.33 14.5,18.26C14.5,19.6 14.5,20.68 14.5,21C14.5,21.27 14.66,21.59 15.17,21.5C19.14,20.16 22,16.42 22,12A10,10 0 0,0 12,2Z"></path></svg>
                            </a>
                        </button>
                    </div>
                    </div>
                </div>

                <div className="card bg-base-100 w-65 shadow-sm max-h-100 scale-75">
                    <figure>
                        <img
                        src={JG}
                        alt="JoaoG" />
                    </figure>
                    <div className="card-body items-center">
                        <h1 className="card-title pb-2 font-gowunBold">João Gonzaga</h1>
                        <div className="card-actions">
                        <button className="btn btn-circle bg-black text-white border-black">
                            <a href="https://github.com/Karmantinedev">
                                <svg aria-label="GitHub logo" width="32" height="32" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="white" d="M12,2A10,10 0 0,0 2,12C2,16.42 4.87,20.17 8.84,21.5C9.34,21.58 9.5,21.27 9.5,21C9.5,20.77 9.5,20.14 9.5,19.31C6.73,19.91 6.14,17.97 6.14,17.97C5.68,16.81 5.03,16.5 5.03,16.5C4.12,15.88 5.1,15.9 5.1,15.9C6.1,15.97 6.63,16.93 6.63,16.93C7.5,18.45 8.97,18 9.54,17.76C9.63,17.11 9.89,16.67 10.17,16.42C7.95,16.17 5.62,15.31 5.62,11.5C5.62,10.39 6,9.5 6.65,8.79C6.55,8.54 6.2,7.5 6.75,6.15C6.75,6.15 7.59,5.88 9.5,7.17C10.29,6.95 11.15,6.84 12,6.84C12.85,6.84 13.71,6.95 14.5,7.17C16.41,5.88 17.25,6.15 17.25,6.15C17.8,7.5 17.45,8.54 17.35,8.79C18,9.5 18.38,10.39 18.38,11.5C18.38,15.32 16.04,16.16 13.81,16.41C14.17,16.72 14.5,17.33 14.5,18.26C14.5,19.6 14.5,20.68 14.5,21C14.5,21.27 14.66,21.59 15.17,21.5C19.14,20.16 22,16.42 22,12A10,10 0 0,0 12,2Z"></path></svg>
                            </a>
                        </button>
                    </div>
                    </div>
                </div>

                <div className="card bg-base-100 w-65 shadow-sm max-h-100 scale-75">
                    <figure>
                        <img
                        src={LF}
                        alt="LucasF" />
                    </figure>
                    <div className="card-body items-center">
                        <h1 className="card-title pb-2 font-gowunBold">Lucas Fujimoto</h1>
                        <div className="card-actions">
                        <button className="btn btn-circle bg-black text-white border-black">
                            <a href="https://github.com/Lucasft16">
                                <svg aria-label="GitHub logo" width="32" height="32" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="white" d="M12,2A10,10 0 0,0 2,12C2,16.42 4.87,20.17 8.84,21.5C9.34,21.58 9.5,21.27 9.5,21C9.5,20.77 9.5,20.14 9.5,19.31C6.73,19.91 6.14,17.97 6.14,17.97C5.68,16.81 5.03,16.5 5.03,16.5C4.12,15.88 5.1,15.9 5.1,15.9C6.1,15.97 6.63,16.93 6.63,16.93C7.5,18.45 8.97,18 9.54,17.76C9.63,17.11 9.89,16.67 10.17,16.42C7.95,16.17 5.62,15.31 5.62,11.5C5.62,10.39 6,9.5 6.65,8.79C6.55,8.54 6.2,7.5 6.75,6.15C6.75,6.15 7.59,5.88 9.5,7.17C10.29,6.95 11.15,6.84 12,6.84C12.85,6.84 13.71,6.95 14.5,7.17C16.41,5.88 17.25,6.15 17.25,6.15C17.8,7.5 17.45,8.54 17.35,8.79C18,9.5 18.38,10.39 18.38,11.5C18.38,15.32 16.04,16.16 13.81,16.41C14.17,16.72 14.5,17.33 14.5,18.26C14.5,19.6 14.5,20.68 14.5,21C14.5,21.27 14.66,21.59 15.17,21.5C19.14,20.16 22,16.42 22,12A10,10 0 0,0 12,2Z"></path></svg>
                            </a>
                        </button>
                    </div>
                    </div>
                </div>

                <div className="card bg-base-100 w-65 shadow-sm max-h-100 scale-75">
                    <figure>
                        <img
                        src={MG}
                        alt="MariaG" />
                    </figure>
                    <div className="card-body items-center">
                        <h1 className="card-title pb-2 font-gowunBold">Maria Gontijo</h1>
                        <div className="card-actions">
                        <button className="btn btn-circle bg-black text-white border-black">
                            <a href="https://github.com/MariaClara-Canuto">
                                <svg aria-label="GitHub logo" width="32" height="32" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="white" d="M12,2A10,10 0 0,0 2,12C2,16.42 4.87,20.17 8.84,21.5C9.34,21.58 9.5,21.27 9.5,21C9.5,20.77 9.5,20.14 9.5,19.31C6.73,19.91 6.14,17.97 6.14,17.97C5.68,16.81 5.03,16.5 5.03,16.5C4.12,15.88 5.1,15.9 5.1,15.9C6.1,15.97 6.63,16.93 6.63,16.93C7.5,18.45 8.97,18 9.54,17.76C9.63,17.11 9.89,16.67 10.17,16.42C7.95,16.17 5.62,15.31 5.62,11.5C5.62,10.39 6,9.5 6.65,8.79C6.55,8.54 6.2,7.5 6.75,6.15C6.75,6.15 7.59,5.88 9.5,7.17C10.29,6.95 11.15,6.84 12,6.84C12.85,6.84 13.71,6.95 14.5,7.17C16.41,5.88 17.25,6.15 17.25,6.15C17.8,7.5 17.45,8.54 17.35,8.79C18,9.5 18.38,10.39 18.38,11.5C18.38,15.32 16.04,16.16 13.81,16.41C14.17,16.72 14.5,17.33 14.5,18.26C14.5,19.6 14.5,20.68 14.5,21C14.5,21.27 14.66,21.59 15.17,21.5C19.14,20.16 22,16.42 22,12A10,10 0 0,0 12,2Z"></path></svg>
                            </a>
                        </button>
                    </div>
                    </div>
                </div>
                
                <div className="card bg-base-100 w-65 shadow-sm max-h-100 scale-75">
                    <figure>
                        <img
                        src={MS}
                        alt="MatheusS" />
                    </figure>
                    <div className="card-body items-center">
                        <h1 className="card-title pb-2 font-gowunBold">Matheus Saraiva</h1>
                        <div className="card-actions">
                        <button className="btn btn-circle bg-black text-white border-black">
                            <a href="https://github.com/apptrx">
                                <svg aria-label="GitHub logo" width="32" height="32" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="white" d="M12,2A10,10 0 0,0 2,12C2,16.42 4.87,20.17 8.84,21.5C9.34,21.58 9.5,21.27 9.5,21C9.5,20.77 9.5,20.14 9.5,19.31C6.73,19.91 6.14,17.97 6.14,17.97C5.68,16.81 5.03,16.5 5.03,16.5C4.12,15.88 5.1,15.9 5.1,15.9C6.1,15.97 6.63,16.93 6.63,16.93C7.5,18.45 8.97,18 9.54,17.76C9.63,17.11 9.89,16.67 10.17,16.42C7.95,16.17 5.62,15.31 5.62,11.5C5.62,10.39 6,9.5 6.65,8.79C6.55,8.54 6.2,7.5 6.75,6.15C6.75,6.15 7.59,5.88 9.5,7.17C10.29,6.95 11.15,6.84 12,6.84C12.85,6.84 13.71,6.95 14.5,7.17C16.41,5.88 17.25,6.15 17.25,6.15C17.8,7.5 17.45,8.54 17.35,8.79C18,9.5 18.38,10.39 18.38,11.5C18.38,15.32 16.04,16.16 13.81,16.41C14.17,16.72 14.5,17.33 14.5,18.26C14.5,19.6 14.5,20.68 14.5,21C14.5,21.27 14.66,21.59 15.17,21.5C19.14,20.16 22,16.42 22,12A10,10 0 0,0 12,2Z"></path></svg>
                            </a>
                        </button>
                    </div>
                    </div>
                </div>

                <div className="card bg-base-100 w-65 shadow-sm max-h-100 scale-75">
                    <figure>
                        <img
                        src={LL}
                        alt="LuanL" />
                    </figure>
                    <div className="card-body items-center">
                        <h1 className="card-title pb-2 font-gowunBold">Luan Ludry</h1>
                        <div className="card-actions">
                        <button className="btn btn-circle bg-black text-white border-black">
                            <a href="https://github.com/luanludry">
                                <svg aria-label="GitHub logo" width="32" height="32" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="white" d="M12,2A10,10 0 0,0 2,12C2,16.42 4.87,20.17 8.84,21.5C9.34,21.58 9.5,21.27 9.5,21C9.5,20.77 9.5,20.14 9.5,19.31C6.73,19.91 6.14,17.97 6.14,17.97C5.68,16.81 5.03,16.5 5.03,16.5C4.12,15.88 5.1,15.9 5.1,15.9C6.1,15.97 6.63,16.93 6.63,16.93C7.5,18.45 8.97,18 9.54,17.76C9.63,17.11 9.89,16.67 10.17,16.42C7.95,16.17 5.62,15.31 5.62,11.5C5.62,10.39 6,9.5 6.65,8.79C6.55,8.54 6.2,7.5 6.75,6.15C6.75,6.15 7.59,5.88 9.5,7.17C10.29,6.95 11.15,6.84 12,6.84C12.85,6.84 13.71,6.95 14.5,7.17C16.41,5.88 17.25,6.15 17.25,6.15C17.8,7.5 17.45,8.54 17.35,8.79C18,9.5 18.38,10.39 18.38,11.5C18.38,15.32 16.04,16.16 13.81,16.41C14.17,16.72 14.5,17.33 14.5,18.26C14.5,19.6 14.5,20.68 14.5,21C14.5,21.27 14.66,21.59 15.17,21.5C19.14,20.16 22,16.42 22,12A10,10 0 0,0 12,2Z"></path></svg> 
                            </a>
                        </button>
                    </div>
                    </div>
                </div>
            </div>
        
                <div className="max-w-3xl mx-auto flex flex-row gap-10 pb-5">
                    <div>
                        <p className="font-gowunBold text-lg md:text-xl wrap-break-words leading-relaxed md:leading-loose mb-6">
                        Como alunos da UnB no campus da FGA, percebemos que não existia um ambiente 
                        que centralizasse informações sobre atividades extras. A busca por diferentes 
                        tipos de eventos ou detalhes era complicada, com materiais dispersos em 
                        diversos arquivos e sites.
                        </p>
                    </div>
                    <div>
                        <p className="font-gowunBold text-lg md:text-xl wrap-break-words leading-relaxed md:leading-loose">
                        Nasceu assim o <strong className="text-primary">Mural UnB</strong>, nosso projeto da disciplina 
                        de Métodos de Desenvolvimento de Software. É uma plataforma intuitiva que 
                        simplifica a busca por atividades e oportunidades, utilizando <strong className="text-secondary">IA para 
                        organização</strong>, sistema de <strong className="text-secondary">tags categorizadas</strong> e uma 
                        <strong className="text-secondary"> ferramenta de pesquisa</strong>.
                        </p>
                    </div>
                </div>

        </div>
    )
}