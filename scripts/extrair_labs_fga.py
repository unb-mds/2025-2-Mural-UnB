...
from bs4 import (
    BeautifulSoup,
)  # Para "ler" e navegar pelo HTML das páginas web              # Para montar URLs completas (juntar URL base com caminhos relativos)
...
from unidecode import (
    unidecode,
)  # Para remover acentos de textos (ex: "Robótica" -> "Robotica")
...
CAMINHO_PLACEHOLDER = os.path.join(
    "..", "data", "images", "placeholders", "default_lab.jpg"
)
...
CATEGORIAS_KEYWORDS = {
    ...
    "mecanica_materiais": [  # Agrupa Automotiva, Aero, Energia, Materiais, Física
        ...
        "controle",
        "robotica",
        "automacao",  # Adicionei Robótica aqui também
    ],
...
def extrair_palavra_chave(nome_do_lab):
    ...
    print(
        f"    [Palavra Chave] Não foi possível extrair chave de '{nome_do_lab}'. Usando 'pesquisa'."
    )
    return "pesquisa"
...
def baixar_imagem(url_imagem, caminho_salvar):
    ...
    try:
        ...
        response = requests.get(
            url_imagem, headers=headers, timeout=15, verify=False, stream=True
        )
        ...
        if not content_type or not content_type.startswith("image/"):
            print(
                f"    [Download] ⚠️ URL não retornou um tipo de conteúdo de imagem válido (recebido: {content_type}). Pulando download."
            )
    ...
    # Tratamento de erros específicos para o download
        ...
        print(
            f"    [Download] ❌ Falha ao baixar {url_imagem}: A requisição demorou demais (Timeout)."
        )
        return False
        ...
        print(
            f"    [Download] ❌ Erro inesperado durante o download/salvamento de {url_imagem}: {e}"
        )
        return False
...
def encontrar_imagem_para_lab(nome_do_lab, pasta_base_imagem):
    ...
    try:
        ...
        for resultado in resultados_da_busca:
            ...
            if keyword_no_titulo or keyword_na_url:
                ...
                if ".unb.br" in url_lower:
                    print(
                        f"    [Busca Imagem] 🎯 Prioritário (UnB + Chave) encontrado: {url_original}"
                    )
                ...
                if not url_fallback:
                    print(
                        f"    [Busca Imagem] ⚠️ Relevante (Externo + Chave) encontrado: {url_original}"
                    )
        ...
        if url_prioritaria:
            ...
            homepage_url = (
                url_ultimo_recurso  # 3º: Primeiro link que não estava na blacklist
            )
        else:
            print(
                "    [Busca Imagem] ❌ Nenhum resultado web parece ser uma homepage válida (todos na blacklist?)."
            )
            return None  # Desiste
        ...
        try:
            ...
            response_lab = requests.get(
                homepage_url, headers=headers, timeout=15, verify=False
            )
            ...
            def is_url_valida(url_teste):
                ...
                if any(
                    nome_ruim in url_lower for nome_ruim in IMAGE_FILENAME_BLACKLIST
                ):
                        ...
                        for img_conteudo in area_conteudo.find_all("img"):
                            if img_conteudo and img_conteudo.get("src"):
                                ...
                                if is_url_valida(url_teste):
                                    ...
                                    width_str = img_conteudo.get("width", "0").replace(
                                        "px", ""
                                    )
                                    height_str = img_conteudo.get(
                                        "height", "0"
                                    ).replace("px", "")
                                    ...
                                    try:
                                        ...
                                        if width >= 200 and height >= 200:
                                            url_imagem_encontrada = url_teste
                                            print(
                                                f"      [Caça] 💚 Verde ('{seletor}' img >= 200px)"
                                            )
                                            break
                                    except ValueError:
                ...
                for seletor in seletores_logo:
                    ...
                    if logo_img and logo_img.get("src"):
                        ...
                        if is_url_valida(url_teste):
                            ...
                            if (
                                not domain_homepage.endswith(".unb.br")
                                and "logo" in url_teste.lower()
                            ):
                                print(
                                    f"      [Caça] ⚠️ Rejeitado (Logo Externo): {url_teste}"
                                )
            ...
            if not url_imagem_encontrada:
                ...
                if header:
                    for img_header in header.find_all(
                        "img"
                    ):  # Busca todas as imgs no header
                        if img_header and img_header.get("src"):
                            url_teste = img_header.get("src")
                            if is_url_valida(url_teste):
                                width_str = img_header.get("width", "0").replace(
                                    "px", ""
                                )
                                height_str = img_header.get("height", "0").replace(
                                    "px", ""
                                )
                                try:
                ...
                if not url_imagem_encontrada:
                    banner = soup_lab.find(
                        "div", class_=lambda x: x and "banner" in x.lower()
                    )
                    if banner:
                        for img_banner in banner.find_all(
                            "img"
                        ):  # Busca todas as imgs no banner
                            if img_banner and img_banner.get("src"):
                                url_teste = img_banner.get("src")
                                if is_url_valida(url_teste):
                                    width_str = img_banner.get("width", "0").replace(
                                        "px", ""
                                    )
                                    height_str = img_banner.get("height", "0").replace(
                                        "px", ""
                                    )
                                    try:
                                        if (
                                            int(width_str) > 100
                                            and int(height_str) > 100
                                        ):
                                            url_imagem_encontrada = url_teste
                                            print(
                                                "      [Caça] 🥉 Bronze (banner div img > 100px)"
                                            )
                                            break
                                    except ValueError:
                                        pass
            ...
            if url_imagem_encontrada:
                ...
                nome_prefixo = "".join(c for c in nome_do_lab if c.isalnum())[
                    :3
                ].lower()
                ...
                print(
                    "      [Caça] ❌ Nenhuma imagem encontrada na página após todas as tentativas."
                )
        ...
        ) as e:  # <--- NÍVEL 2 (ALINHADO COM O TRY INTERNO)
            print(
                f"    [Busca Imagem] ❌ Erro de conexão/HTTP ao acessar homepage {homepage_url}: {e}"
            )

        # --- LINHA DO ERRO ---
        # Esta linha deve estar no NÍVEL 1, (ALINHADA COM O TRY EXTERNO)
        print(
            "    [Busca Imagem] ❌ Falha geral ao encontrar/baixar imagem para este laboratório."
        )
        return None
...
def extrair_laboratorios_fga_pdf(pdf_path, pagina_inicial=13):
    ...
    while i < len(linhas):
        ...
        if re.match(
            r"^\d+\.\d+(\.\d+)*\.", linha
        ) or re.match(  # Numeração de seção (1.2.5.)
            r"^[_\-]{20,}$", linha
        ):  # Linha de separação longa
        ...
        if re.match(r"^(\d+)\.$", linha):
            ...
            if i + 1 < len(linhas):
                ...
                if (
                    proxima_linha
                    and not proxima_linha.startswith("COORDENADOR:")
                    and not proxima_linha.startswith("COORDENADORES:")
                    and not proxima_linha.startswith("CONTATO:")
                    and not proxima_linha.startswith("DESCRIÇÃO:")
                    and not proxima_linha.startswith("DESCRICAO:")
                ):
        ...
        if padrao_numero_simples:
            ...
            palavras_significativas = [
                p for p in nome_sem_numero.split() if len(p) > 2 and p.isalpha()
            ]
            ...
            if len(nome_sem_numero) > 10 and len(nome_sem_numero) < 200:
                ...
                if nome_sem_numero.endswith("-") and i + 1 < len(linhas):
                    ...
                    if (
                        proxima
                        and len(proxima) < 30
                        and not proxima.startswith("COORDENADOR:")
                        and not proxima.startswith("COORDENADORES:")
                        and not proxima.startswith("CONTATO:")
                        and not proxima.startswith("DESCRIÇÃO:")
                        and not proxima.startswith("DESCRICAO:")
                    ):
        ...
        # Se estamos rastreando um laboratório, tenta preencher informações
            ...
            if (
                linha.startswith("COORDENADOR:")
                or linha.startswith("COORDENADORES:")
                or linha.startswith("RESPONSÁVEL:")
                or linha.startswith("RESPONSÁVEIS:")
            ):
                coordenador_texto = (
                    linha.split(":", 1)[1].strip() if ":" in linha else ""
                )
                # Remove todos os IDs Lattes (pode haver múltiplos)
                coordenador_texto = re.sub(
                    r"\s*\(ID Lattes:\s*\d+\)",
                    "",
                    coordenador_texto,
                    flags=re.IGNORECASE,
            ...
            # DESCRIÇÃO
                ...
                while j < len(linhas):
                    ...
                    if proxima_linha and re.match(
                        r"^\d+\.\d+(\.\d+)*\.", proxima_linha
                    ):
                        break
                    ...
                    if proxima_linha and (
                        ":" in proxima_linha
                        and any(
                            proxima_linha.startswith(palavra)
                            for palavra in [
                                "GRUPOS",
                                "EQUIPAMENTOS",
                                "COORDENADOR",
                    ...
                    if proxima_linha:
                        ...
                        if re.match(
                            r"^[IVX]+\s*-\s*[A-ZÀÁÂÃÉÊÍÓÔÕÚÇ\s]+$", proxima_linha
                        ):
                            break
                        # Detecta rodapé institucional
                        if any(
                            palavra in proxima_linha.upper()
                            for palavra in [
                ...
                descricao = re.sub(
                    r"\s+CLASSIFICA[ÇC][ÃA]O:.*$", "", descricao, flags=re.IGNORECASE
                )
                descricao = re.sub(
                    r"\s+Laborat[óo]rio de Pesquisa\s*$",
                    "",
                    descricao,
                    flags=re.IGNORECASE,
                )
                ...
                descricao = re.sub(
                    r"\s+[IVX]+\s*-\s*CIÊNCIAS.*$", "", descricao, flags=re.IGNORECASE
                )
                descricao = re.sub(
                    r"\s+UNIVERSIDADE DE BRASÍLIA.*$",
                    "",
                    descricao,
                    flags=re.IGNORECASE,
                )
                descricao = re.sub(
                    r"\s+PORTFÓLIO.*$", "", descricao, flags=re.IGNORECASE
                )
                # Remove hifenização de quebra de linha
...
def filtrar_labs_fga(pdf_path, csv_saida):
    ...
    for lab in labs_fga_sem_id:  # <--- USA A VARIÁVEL DA 'main'
        ...
        caminho_imagem_local = encontrar_imagem_para_lab(
            lab["nome"], PASTA_IMAGENS_LABS
        )

        if caminho_imagem_local:
            lab["caminho_imagem"] = os.path.join(
                "..", "images", "labs", os.path.basename(caminho_imagem_local)
            )
            ...
            if categoria in ["software", "eletronica", "mecanica_materiais", "default"]:
                ...
                categoria = "default"
            lab["caminho_imagem"] = os.path.join(
                "..", "data", "images", "placeholders", nome_placeholder
            )
            print(
                f"---> Usando placeholder ({categoria} variação {numero_variacao}): {lab['caminho_imagem']}"
            )
...
def main():
    ...
    pdf_path = os.path.join(
        script_dir, "..", "data", "Labs", "Portfolio_Infraestrutura_UnB.pdf"
    )
